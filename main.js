...
class GfxFont{
    ...
    constructor(parsedFont){
        ...
        this.first = parsedFont.first;
        this.last = parsedFont.last;
    ...
    resize(first, last){
        if(first < this.first) {
            this.glyphs.unshift(...new Array(this.first - first).fill(0).map((_, i) => new GfxGlyph(String.fromCharCode(first + i))));
        } else if(first > this.first) {
            this.glyphs.splice(0, first - this.first);
        }
        if(last > this.last) {
            this.glyphs.push(...new Array(last - this.last).fill(0).map((_, i) => new GfxGlyph(String.fromCharCode(this.last + 1 + i))));
        } else if(last < this.last) {
            this.glyphs.splice(last - this.first + 1);
        }
        this.first = first;
        this.last = last;
    }
...
const font = new GfxFont(parsedFont);
